...
export const normalizePattern = (pattern: any, size: number = 8): Pattern | null => {
  if (isValidPattern(pattern)) {
    if (pattern.length === size) {
      return pattern
    }
    
    const normalized: Pattern = Array(size).fill(null).map(() => Array(size).fill(false))
...
export const stringToPattern = (str: string, size: number = 8): Pattern | null => {
...
export const compressPattern = (pattern: Pattern): string => {
  const binary = pattern.flat().map(cell => cell ? '1' : '0').join('')
  const bytes: number[] = []
...
export const decompressPattern = (compressed: string, size: number = 8): Pattern | null => {
  try {
